...
| Teil | Status | Details |
|---|---|---|
| Backend (FastAPI) | ✅ Fertig & getestet | **279/279 pytest-Tests grün** (`docker compose exec backend pytest tests/ -v`; von frischem Postgres-Volume aus verifiziert, inkl. RAG-Pfad). Auth, Tenants, POIs (inkl. Delete), Trips, Chat mit echtem RAG (pgvector-Similarity-Search über `pois.embedding`), Content-Pipeline, Analytics, Booking-Agent mit Consent-Flow, **Review-Flow (CRUD + Aggregat-Automatisierung)**, **CV-Vision über denselben Provider-Pfad wie Chat/RAG**, **Constraint-Solver (17 Tests)** — siehe Prinzip 6. **Review-Modell & Aggregat-Sync** (`app/models.py` + `app/services/review_service.py`): tenant-skopiertes Rating/Comment pro POI, `UniqueConstraint("tenant_id", "user_id", "poi_id")` verhindert Mehrfach-Reviews; `POI.average_rating`/`review_count` werden bei jedem Create/Update/Delete via `recompute_poi_rating()` aktuell gehalten und über die API exponiert (`app/api/v1/endpoints/reviews.py`, 7 Tests). **Bugfix:** `User`-E-Mail-Eindeutigkeit pro Tenant war zuvor nur ein SQL-Kommentar ohne Wirkung, jetzt eine echte `UniqueConstraint("tenant_id", "email")` — durch `test_users.py` (2 Tests) abgesichert. Alle `datetime.utcnow`-Defaults laufen jetzt über eine benannte `utcnow()`-Hilfsfunktion (vermeidet das SQLAlchemy-Fallstrick, einen Zeitpunkt einmalig beim Modul-Import statt bei jedem Insert zu berechnen). **LLM-Provider-Architektur inkl. Embeddings & Vision** (Prinzip 7): `app/services/llm_providers/` mit `LLMProvider`-Base (`complete()`, `embed()`, `identify_image()`), `OpenAIProvider`, `MockProvider` (deterministisches Fake-Embedding und -Vision, kein Netzwerk-Call), `get_llm_provider()`-Factory. CV (`/cv/identify`) ist auf diesen Provider-Pfad migriert via `app/services/cv_service.py` — echte OpenAI-Vision mit API-Key, sonst deterministischer Mock (3 Tests in `test_cv.py`). Aktivierung über `LLM_PROVIDER=openai` + `OPENAI_API_KEY` in `backend/.env` (nicht versioniert) — ohne beides läuft alles deterministisch im Mock-Modus. RAG zusätzlich gated über `USE_PGVECTOR` (Default `false`). `deploy/postgres/init-test-db.sql` aktiviert die `vector`-Extension beim ersten Volume-Init — **Postgres-Image muss nach Änderungen an diesem Script neu gebaut werden** (`docker compose build postgres`), sonst bleibt die alte Version im Image. **Hinweis Testcount:** real 279 Testfunktionen (Stand 2026-09-26, `python3 -m pytest -q --no-cov` im Ordner `backend` gegen den laufenden Postgres-Container). **KI-Quota pro Tenant** (`TenantAIQuotaConfig`/`TenantAIUsageLog` + `app/services/tenant_ai_quota_service.py`): Default 50 USD/Monat, Soft-Alert ab 80 %, Hard-Limit aktiv. Die Abrechnungsperiode wird *faul* zurückgesetzt (`_reset_period_if_expired`, kein Cron): die erste Lesung nach Ablauf setzt `current_period_usage=0`, damit ein Überschreiten des Vormonats den Tenant nicht dauerhaft aussperrt. `record_usage()` schreibt den Usage-Log-Eintrag **und** erhöht `current_period_usage` per atomarem `UPDATE ... SET x = x + :cost` in derselben Transaktion (nie read-then-write, sonst verlorene Inkremente bei parallelen Calls). `get_tenant_llm_provider()` prüft das Quota für **alle** Tenants — auch ohne `TenantAIConfig`, also im Plattform-Fallback — und wirft `QuotaExceededError` vor jedem Provider-Aufruf. Die Anlage der Quota-Zeile ist rennsicher: `get_or_create_quota_config` fängt den `IntegrityError` beim `commit()` ab (zwei gleichzeitige *erste* KI-Aufrufe eines Tenants sehen beide „keine Zeile"), rollt die abgebrochene Transaktion zurück und liest die Zeile der anderen Anfrage nach — statt den KI-Aufruf des Nutzers abzubrechen. (23 Tests: 16 in `test_tenant_ai_quota_service.py` mit `FakeDB`, 3 in `test_tenant_ai_quota_db.py` gegen echtes Postgres — darunter ein Rennen, das eine `Gate`-Barriere auf dem `commit` deterministisch erzwingt und die Zahl der von Postgres tatsächlich abgelehnten INSERTs selbst nachzählt (`handle_error`-Listener am Engine, also unabhängig vom Service-Code): ohne die Barriere fällt der Test sofort durch (0 Konflikte) statt still nichts zu prüfen —, 4 in `test_tenant_llm_provider.py`.) **Schema-Hinweis:** `Review`-Tabelle und neue Spalten auf `POI`/`User` erfordern bei bestehenden Daten eine echte Migration (Alembic) statt `create_all` — in Dev/Test unkritisch, da Volumes bei Schema-Änderungen neu erstellt werden. |
| Frontend (Next.js) | ✅ Fertig & E2E-getestet | **13/13 Playwright-E2E-Tests grün** (`docker compose --profile test run --rm playwright`) — Landing-Page, Login/Register-Flow, Demo-Login, alle 5 Tabs (Home/Explorer/Réservations/Assistant/Profil), Logout, Fehlerbehandlung bei ungültigem Token. Single-Page-App mit genau einer Next.js-Route (`/`), Tabs über React-State in `AppShell.tsx`. `next.config.mjs` proxied `/api/*` serverseitig zum Backend (Rewrite, konfigurierbar über `BACKEND_URL`). **Sicherheit:** `next` auf `16.3.2` aktualisiert (Major-Upgrade von v14 → v16, überspringt v15); `next-intl` entfernt; ESLint-Konfiguration von `.eslintrc.json` auf flache Konfiguration `eslint.config.mjs` migriert; `frontend/src/middleware.ts` → `frontend/src/proxy.ts` umbenannt; `frontend/next.config.js` → `frontend/next.config.mjs`. Frühere `high`-Findings in `npm audit` (`glob`/`minimatch`/`eslint-config-next`) wurden durch das Major-Upgrade auf `next@16.x` behoben — keine offenen Sicherheitsrisiken mehr. |
...
- `frontend/next.config.mjs` (ursprünglich `next.config.js`): API-Rewrite-Destination war fest auf `http://localhost:8000` verdrahtet — im Docker-Netzwerk unerreichbar. Jetzt konfigurierbar über `process.env.BACKEND_URL`.
...
**Hinweis zur Pflege dieser Tabelle:** Der Testcount ändert sich häufig. Vor größeren Sessions `python3 -m pytest -q --no-cov` im Ordner `backend` laufen lassen und die Zahl in der Backend-Zeile anpassen, statt sie aus dieser Tabelle zu übernehmen.
...
## Tenant-Administration (Stand 2026-09-20)

- **Admins gehören genau einem Tenant.** Der JWT-Claim `tenant` muss zu `X-Tenant-Slug` passen (sonst 403 „Tenant mismatch“). Routen der Form `/tenants/{tenant_id}/...` verwenden `get_tenant_admin` (`app/dependencies.py`) und antworten mit 404, wenn `tenant_id` nicht der Tenant des Admins ist. Es gibt bewusst **keinen Super-Admin auf Plattformebene**.
- **`GET /tenants/` liefert nur den Tenant des angemeldeten Admins.**
- **Tenants werden nicht über die API angelegt.** `POST /tenants/` existiert nicht mehr (405). Neuer Tenant plus erster Admin, atomar: `python -m app.create_tenant --slug ... --name ... --language ... --currency ... --admin-email ...` (im Container: `docker compose exec backend python -m app.create_tenant ...`). Das Passwort wird interaktiv abgefragt oder aus `TENANT_ADMIN_PASSWORD` gelesen. Phase 4 (weitere Länder) = dieses Skript plus Content/Config, keine Kernentwicklung.
- **Neue Routen unter `/{tenant_id}/...` müssen `get_tenant_admin` verwenden, nie `get_current_admin` allein.** Ein Isolationstest ist Pflicht; Muster: die Fixtures `other_tenant`, `other_admin`, `other_admin_headers` in `backend/tests/conftest.py`.

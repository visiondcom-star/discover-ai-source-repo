...
| Teil | Status | Details |
|---|---|---|
| Backend (FastAPI) | ✅ Fertig & getestet | **61/61 pytest-Tests grün** (`docker compose exec backend pytest tests/ -v`; von frischem Postgres-Volume aus verifiziert). Auth, Tenants, POIs (inkl. Delete), Trips, Chat, Content-Pipeline, Analytics, Booking-Agent mit Consent-Flow, CV-Stub, **Constraint-Solver (17 Tests)** — siehe Prinzip 6. **LLM-Provider-Architektur umgesetzt** (Prinzip 7): `app/services/llm_providers/` mit `LLMProvider`-Base, `OpenAIProvider`, `MockProvider`, `get_llm_provider()`-Factory; läuft ohne API-Key deterministisch im Mock-Modus. **Hinweis Testcount:** Dokumente nennen teils „78" — dieses Repo enthält real 61 Testfunktionen (kein `test_rag.py`/`test_context.py`/`test_cv.py` in dieser Kopie). |
| Frontend (Next.js) | ✅ Fertig & E2E-getestet | **13/13 Playwright-E2E-Tests grün** (`docker compose --profile test run --rm playwright`) — Landing-Page, Login/Register-Flow, Demo-Login, alle 5 Tabs (Home/Explorer/Réservations/Assistant/Profil), Logout, Fehlerbehandlung bei ungültigem Token. Single-Page-App mit genau einer Next.js-Route (`/`), Tabs über React-State in `AppShell.tsx`. `next.config.js` proxied `/api/*` serverseitig zum Backend (Rewrite, konfigurierbar über `BACKEND_URL`). |
| Mobile (Flutter) | 🟡 Teils begonnen | Es existieren **zwei** Flutter-Projekte: `flutter_app/` (eigene App mit `test/widget_test.dart`, iOS/Android/Web/Linux/macOS/Windows-Scaffold) und `mobile/` (ältere Variante mit `lib/{models,providers,screens,widgets,services}` + 3 Testdateien). Beide sind nicht als produktiv verifiziert — vor weiterer Arbeit prüfen, welche Variante die Referenz sein soll, und die andere entfernen. |
...
| LLM/CV-Anbindung | ✅ Abstrahiert | **LLM-Provider-Abstraktion fertig** (siehe Backend-Zeile). CV: `app/api/v1/endpoints/cv.py` implementiert (`identify_object`, `get_ar_assets`, `get_nearby_ar`). Läuft ohne API-Key im deterministischen Mock-Modus. |
...
4. **Flutter-Duplikat auflösen** — `mobile/` vs `flutter_app/`: Referenz bestimmen, eine Variante entfernen; dann Flutter-App produktiv aufbauen (als eigene Teilaufgabe)
